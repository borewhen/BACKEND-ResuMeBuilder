...
def handle_resume_upload(user_id: int, file_bytes: bytes | None, filename: str | None, db: Session) -> str:
    ...
    prompt = f"""
    You are a mock interviewer. Based on this resume, generate a first interview question. The question can be technical or behavioral.
    - Make the question specific to the candidate's projects, companies, or skills.
...
'''
# --- Handle Answer & Generate Next Question ---
def handle_answer(user_id: int, answer: str, db: Session):
    prev_q = db.query(VideoInterviewQuestion).filter_by(user_id=user_id, answer=None).order_by(VideoInterviewQuestion.id).first()
    if not prev_q:
